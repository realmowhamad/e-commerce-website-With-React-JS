    ...
    if (products.length > 0) return (
        <div className="home__container w-screen h-screen overflow-y-scroll">
            <section className={`${style.mainSection} w-screen h-screen overflow-hidden flex flex-col p-2  laptop:flex-row-reverse relative`}>
                <div className=" h-2/6 rounded-t-[10px] items-center flex flex-col text-white font-semibold text-2xl mt-2 relative  laptop:text-gBlack laptop:items-center laptop:justify-start laptop:h-full laptop:text-[50px] ">
                    ...
                    <button className="w-3/6 text-xl px-5 py-2 mt-8 rounded-[10px] laptop:bg-light text-gBlack laptop:text-white laptop:w-2/6 laptop:py-3 laptop:mt-20 " onClick={scrollHandler}>shop now</button>
                </div>
                <img className="laptop:aspect-auto bottom-0" src={modelImage} alt="" />
                <div className="flex text-[20px] top-[30%] h-1/6 text-white flex-col justify-around laptop:flex-row w-2/6 absolute laptop:top-[80%] laptop:left-[55%] laptop:text-[50px] laptop:text-gray-400 ">
                    <Link className="hover:text-lightGray" to={''}><i><AiFillInstagram /></i></Link>
                    <Link className="hover:text-lightGray" to={''}><i><AiFillFacebook /></i></Link>
                    <Link className="hover:text-lightGray" to={''}><i><BsTelegram /></i></Link>
                    <Link className="hover:text-lightGray" to={''}><i><AiFillTwitterCircle /></i></Link>
                </div>
